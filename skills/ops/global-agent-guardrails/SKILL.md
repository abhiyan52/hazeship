...
> Vendored unmodified from [davidondrej/skills](https://github.com/davidondrej/skills)
> (MIT license, see `LICENSE` in this folder). Update by re-pulling from
> `skills/ops-and-setup/global-agent-guardrails/` in that repo rather than
> hand-editing this copy.
>
> **This skill installs to the user's machine (`~/.agents/hooks/`), not to a
> project.** It's the one exception to every other hazeship skill, which
> runs scoped to a repo. hazeship ships the three hook files this skill
> references at its own repo root, `hooks/` — mirroring the source repo's
> layout so the "State check" and "Add or tune a pattern" sections below
> work unmodified. To install: copy (or symlink) hazeship's `hooks/` to
> `~/.agents/hooks/`, then wire it into each agent per the "Per-agent
> wiring" table below — this skill does not do that wiring for you, since
> the target paths and configs are the user's own agent installs, not
> anything hazeship manages.
