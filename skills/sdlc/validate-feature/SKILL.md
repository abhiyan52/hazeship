...
`docs/templates/validation.md`. Read the manifest, PRD, gap analysis, tech
design, dev plan, and ALL slice logs first — validation is judged against the
requirements, not against what the slices happened to build.
...
2. Verify each row by the strongest available method, in order of preference:
   ...
   evidence.
3. Run the project's own compliance/data-handling verification checklist
   over the full feature diff (`git diff <staging>...feat/<slug>`).
...
5. Verdict: PASS only if every requirement row passes and no unaccepted
   compliance failures exist. Otherwise FAIL with the exact failing rows —
