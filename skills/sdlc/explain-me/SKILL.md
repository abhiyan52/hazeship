...
description: |
  ...
  works, grounded in the feature's actual artifacts and code — the SDLC's
  version of a generic teach skill. Invocable at ANY point in the SDLC — use
  when the user says "I'm confused", "explain what we're building", "teach
  me how this works", "why did we design it this way", or "walk me through
  this slice".
---

# Explain Me (SDLC-grounded teaching)

## Usage

```
/explain-me <what are you confused about?>
```

If a generic `teach` skill is installed in your global skills, its
lesson-authoring mechanics (mission format, lesson/reference/asset
structure, diagrams, knowledge vs skills, quizzes, wisdom/community) take
precedence for consistency across projects — read it and follow it, layered
under the SDLC rules below. This kit does not ship that skill, so the rules
below are also complete on their own when it isn't installed.
...
A generic teach skill asks "why do you want to learn this?". Here the
mission is usually inferable: read the feature's `manifest.yaml` (status + latest
...
3. **Author lessons** as self-contained HTML in `lessons/`, numbered
   ...
   - **Diagrams: `archify`, never Mermaid.** Read the feature's tech-design
     diagrams (or the real code, for architecture the tech design doesn't
     cover) for topology and meaning, then author a fresh Archify JSON spec
     — never redraw reality from memory, and never mechanically port Mermaid
     styling. Pick the type from archify's router (golden flow →
     `workflow`/`sequence`; components → `architecture`; a lifecycle →
     `lifecycle`). Per diagram, saved to `diagrams/<name>.{json,html,png}`:
     author the JSON → `archify deliver <type> diagrams/<name>.json
     diagrams/<name>.html --quality showcase --json` (must exit 0, 0
     composition errors) → open `diagrams/<name>.html?theme=light` (forces
     the light palette) → export PNG via the toolbar (`#btn-export` then
     `button[data-format="png"]`) → save as `diagrams/<name>.png` and
     confirm it is light-background. Embed the PNG in the lesson HTML; the
     `.html`/`.json` are regenerable working files, not part of the lesson.
     If `archify` isn't installed, fall back to a plain SVG/HTML diagram
     rather than Mermaid.
