...
│   ├── learning/              # cross-cutting lessons from /explain-me
...
`implement-slice`), `*.docx` (regenerable from the markdown),
`docs/features/**/diagrams/*.html` and `docs/features/**/diagrams/*.json`
(archify build artifacts regenerated by `/to-technical-doc` — only the
exported `diagrams/*.png` is committed), and `seeds/**/output/` (generated
dumps and exports).
